...
public class PractitionerMutation
{
    ...
    public async Task<PractitionerCreation> CreatePractitioner(PractitionerInput practitioner)
    {
        try
        {
            Practitioner? newPractitioner = InputConvert.ToPractitioner(practitioner);
            Practitioner? result = await _fhirClient.CreateAsync(newPractitioner);
    ...
    public async Task<PractitionerUpdate> UpdatePractitioner(string id, PractitionerInput practitioner)
    {
        try
        {
            Bundle? find = await _fhirClient.SearchByIdAsync<Practitioner>(id);
            if (find == null)
            {
                // the resource was not found
                throw new Exception("Item was not found.");
            }
            Practitioner? existingPractitioner = find.Entry.Select(p => (Practitioner)p.Resource).First();
            Practitioner? updatePractitioner = InputConvert.ToPractitioner(practitioner);
            ...
            Practitioner? result = await _fhirClient.UpdateAsync(existingPractitioner);
    ...
    public async Task<PractitionerDelete> DeletePractitioner(string id)
    {
        try
        {
            ...
            Practitioner? itemToDelete = find.Entry.Select(p => (Practitioner)p.Resource).First();
